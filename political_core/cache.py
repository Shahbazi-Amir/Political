...
class SQLiteCache:
    def __init__(self,path:str|Path=".political-cache.sqlite3",max_rows:int=20_000)->None:
        ...
        with self._connect() as conn:
            conn.execute("PRAGMA journal_mode=WAL");conn.execute("CREATE TABLE IF NOT EXISTS fact_cache(cache_key TEXT PRIMARY KEY,created_at INTEGER NOT NULL,payload TEXT NOT NULL)");conn.execute("CREATE INDEX IF NOT EXISTS idx_fact_cache_created ON fact_cache(created_at)")
    def _connect(self)->sqlite3.Connection:
        conn=sqlite3.connect(self.path,timeout=5.0);conn.execute("PRAGMA busy_timeout=5000");return conn
    def get(self,key:str,ttl_seconds:int)->dict[str,Any]|None:
        ...
        created,payload=row
        if int(time.time())-int(created)>max(0,int(ttl_seconds)):self.delete(key);return None
        try:
            data=json.loads(payload);return data if isinstance(data,dict) else None
        except (json.JSONDecodeError,TypeError,ValueError):self.delete(key);return None
    ...
    def prune(self)->int:
        with self._connect() as conn:
            ...
            if extra:conn.execute("DELETE FROM fact_cache WHERE cache_key IN (SELECT cache_key FROM fact_cache ORDER BY created_at ASC LIMIT ?)",(extra,))
            return extra
