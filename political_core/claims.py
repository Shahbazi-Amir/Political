...
def _requirements(ctype: ClaimType, claim_id: str, negative: bool, current: bool) -> list[EvidenceRequirement]:
    ...
    def add(rt: RequirementType, mandatory: bool=False, preferred: bool=True) -> None:
...
def analyze_claims(text: str, *, reference_date: datetime|None=None, registry: EntityAliasRegistry|None=None, decomposer: ClaimDecomposer|None=None, allow_model_decomposition: bool=False) -> list[Claim]:
    ...
    for idx, part in enumerate(parts, 1):
        # Re-classify each atomic claim locally. The whole-input intent remains useful
        # only as a fallback; otherwise one legal/appointment clause can poison all atoms.
        ...
        refs = extract_entities(part, registry); dinfo = parse_date_text(part, ref)
...
def _query_candidates(claim: Claim, registry: EntityAliasRegistry) -> list[SearchQuery]:
    q=claim.atomic_text[:290]; items=[]
    def add(text: str, purpose: str, priority: int) -> None: items.append(SearchQuery(text[:350], purpose, claim.claim_id, priority))
    if claim.is_negative: add(f'{q[:220]} حکم OR سند OR آرشیو OR انتصاب', "negative_existence", 5)
    elif claim.claim_type in {ClaimType.APPOINTMENT, ClaimType.MEMBERSHIP}: add(f'{q[:220]} "حکم انتصاب" OR "متن حکم" OR "اعضای"', "primary", 5)
    elif claim.claim_type in {ClaimType.LEGAL, ClaimType.CONSTITUTIONAL}: add(f'{q[:220]} "متن قانون" OR "قانون اساسی" OR "اصل"', "primary", 5)
    elif claim.claim_type == ClaimType.QUOTE: add(f'{q[:220]} "متن کامل" OR "ویدئو" OR "رونوشت"', "primary", 5)
    elif claim.current_status: add(f'{q[:220]} جدیدترین OR اکنون OR فعلی', "freshness", 5)
    else: add(q, "neutral", 10)
    add(q, "neutral", 20)
    if claim.current_status or claim.claim_type in {ClaimType.APPOINTMENT, ClaimType.MEMBERSHIP}: add(f'{q[:220]} جایگزین OR عزل OR استعفا OR تمدید', "replacement", 25)
    add(f'{q[:230]} تکذیب OR نادرست OR خلاف', "challenge", 30)
    add(f'{q[:230]} تایید OR تأیید OR سند', "support", 40)
    if claim.is_negative: add(f'{q[:220]} آرشیو OR فهرست اعضا OR سوابق', "archive", 15)
    variants=[]
    ...
    if latin: add(f'{" ".join(latin[:2])} {q[:170]}', "transliteration", 45)
    return items
...
def finalize_coverage(coverage: list[ClaimResearchCoverage], claims: Sequence[Claim]) -> list[ClaimResearchCoverage]:
    ...
    for cov in coverage:
        ...
        # Deliberately do NOT promote freshness/negative-existence into primary.
        # Coverage must describe what was actually searched successfully.
