...
def _engine()->FactCheckEngine:
    ...
    settings=Settings();cache=SQLiteCache(settings.cache_path,settings.cache_max_rows)
    search=CachedSearchProvider(SearxNGSearchProvider(searx,settings.fetch_timeout,settings.max_response_bytes),cache,settings.search_cache_ttl)
    fetcher=CachedFetcher(SafeHttpFetcher(settings.fetch_timeout,settings.max_response_bytes),cache,settings.fetch_cache_ttl)
    policy=SourcePolicy(authority_registry=settings.authority_registry())
    reasoner=OpenAIReasoningProvider(model,max_output_tokens=int(os.getenv("OPENAI_MAX_OUTPUT_TOKENS","1200")),timeout=settings.reasoning_timeout,max_retries=settings.reasoning_retries)
