...
class Settings:
    cache_path:str=os.getenv("CACHE_PATH",".political-cache.sqlite3");fetch_timeout:float=_float("FETCH_TIMEOUT",8.0);max_response_bytes:int=_int("MAX_RESPONSE_BYTES",1_500_000);reasoning_timeout:float=_float("REASONING_TIMEOUT",30.0);reasoning_retries:int=min(1,_int("REASONING_RETRIES",1));store_user_content:bool=os.getenv("STORE_USER_CONTENT","false").lower()=="true"
