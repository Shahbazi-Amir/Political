...
class EntityAliasRegistry:
    ...
    def add(self, canonical: str, *aliases: str) -> None:
        self.aliases.setdefault(normalize_text(canonical), set()).update(normalize_text(x) for x in aliases if x)
    ...
    def variants(self, text: str) -> list[str]:
        ...
        seen=[]
        for x in out:
            if x and normalized_key(x) not in {normalized_key(y) for y in seen}: seen.append(x)
        return seen[:8]
...
def extract_entities(text: str, registry: EntityAliasRegistry | None=None) -> list[EntityRef]:
    ...
    for q in re.findall(r"[«\"]([^»\"]{2,100})[»\"]", t):
        et="organization" if any(h in q for h in _ORG_HINTS) else "unknown"; candidates.append((q,et,.72))
    for canonical, aliases in registry.aliases.items():
        for alias in {canonical,*aliases}:
            if normalize_text(alias) and normalize_text(alias) in t: candidates.append((alias,"organization",.95))
    ...
    for surface,etype,conf in candidates:
        canonical=registry.canonicalize(surface); key=(etype,normalized_key(canonical))
