...
def evaluate_jsonl(path:str|Path)->dict:
    ...
    for r in verified:
        ...
        bins[label]["n"]+=1;bins[label]["correct"]+=is_acceptable
        cited=set(r.get("citation_ids",[]));available=set(r.get("available_evidence_ids",[]));required=bool(r.get("citation_required",bool(cited)));citation_ok+=int(cited.issubset(available) and (not required or bool(cited)))
