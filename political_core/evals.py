...
_CRITICAL_TAGS={"appointment","current_status","negative","quote","legal","breaking_news","copied_sources","conflicting_sources"}
...
def evaluate_jsonl(path:str|Path)->dict:
    rows=list(_iter_cases(path));valid_verdicts={v.value for v in Verdict};verified=[];invalid=[]
    for line_no,r in rows:
        # Missing review_status must NEVER silently become verified.
    ...
    correct=acceptable=high_total=high_correct=false_high=citation_ok=0;bins=defaultdict(lambda:{"n":0,"correct":0});searches=[];reasoning=[];latencies=[];coverages=[];primary_scores=[];independence_scores=[];tags=Counter();independent_review=True
    for r in verified:
        actual=r["actual_verdict"];expected=r["expected_verdict"];accepted=set(r.get("acceptable_verdicts") or [expected]);is_correct=actual==expected;is_acceptable=actual in accepted;correct+=is_correct;acceptable+=is_acceptable;conf=float(r.get("confidence",0));conf=min(1,max(0,conf))
        ...
        cited=set(r.get("citation_ids",[]));available=set(r.get("available_evidence_ids",[]));required=bool(r.get("citation_required",bool(cited)))
        citation_ok+=int(cited.issubset(available) and (not required or bool(cited)))
    ...
    category_ready=all(tags.get(tag,0)>=5 for tag in _CRITICAL_TAGS);sample_ready=n>=100 and category_ready
    # Statistical sample size alone is not enough; production accuracy is only
    # marked established after explicit independent human review of all verified cases.
    production_ready=sample_ready and independent_review
    return {"total_cases":len(rows),"verified_cases":n,"invalid_verified_cases":invalid,"benchmark_sample_sufficient":sample_ready,"critical_tag_counts":{k:tags.get(k,0) for k in sorted(_CRITICAL_TAGS)},"production_accuracy_established":production_ready,"verdict_accuracy":round(correct/n,4),"acceptable_verdict_accuracy":round(acceptable/n,4),"high_confidence_accuracy":round(high_correct/high_total,4) if high_total else None,"false_high_confidence_rate":round(false_high/high_total,4) if high_total else None,"citation_validity":round(citation_ok/n,4),"primary_source_precision_f1":_avg([x for x in primary_scores if x is not None]),"source_independence_accuracy":_avg(independence_scores),"coverage_rate":_avg(coverages),"average_search_queries":_avg(searches),"average_reasoning_calls":_avg(reasoning),"average_latency":_avg(latencies),"calibration_bins":calibration,"note":None if production_ready else "Production political accuracy is not yet statistically established."}
