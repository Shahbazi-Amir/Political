...
class AuthorityRegistry:
    ...
    def authority_for(self, domain: str) -> str | None:
        ...
        if d.endswith(".mil") or ".mil." in d:
            return d
        if d.endswith(".int") or d.endswith(".europa.eu"):
            return d
        return None
...
class PrimarySourceAssessor:
    ...
    def assess(self, result: SearchResult, excerpt: str = "") -> PrimarySourceAssessment:
        ...
        authority = self.authority_registry.authority_for(domain)
        text = normalize_text(f"{result.title} {result.snippet} {excerpt[:1800]}").casefold()
        path = urlsplit(result.url).path.casefold()
        doc_path = any(h in path for h in _DOC_PATH_HINTS)
    ...
    def official_statement_likely(self, result: SearchResult, excerpt: str = "") -> bool:
        domain = domain_of(result.url)
        if not self.authority_registry.authority_for(domain):
            return result.source_kind == SourceKind.OFFICIAL_STATEMENT
