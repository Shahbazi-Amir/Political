...
def _quote_overlap(a:str,b:str)->float:
    qa=set(re.findall(r'["«]([^"»]{12,180})["»]',a))
    qb=set(re.findall(r'["«]([^"»]{12,180})["»]',b))
    if not qa or not qb:return 0.0
    na={normalize_text(x).casefold() for x in qa}
    nb={normalize_text(x).casefold() for x in qb}
...
def build_source_graph(evidence:Sequence[Evidence])->ProvenanceGraph:
    graph=ProvenanceGraph()
    for i in range(len(evidence)):
        for j in range(i+1,len(evidence)):
            a,b=evidence[i],evidence[j]
            if a.cited_source and b.cited_source and normalize_text(a.cited_source).casefold()==normalize_text(b.cited_source).casefold():
                graph.add(i,j,"same_explicit_source",.99,"same cited_source")
                continue
...
def assign_source_chains(evidence:Sequence[Evidence],similarity_threshold:float=.74)->list[Evidence]:
    items=list(evidence)
    graph=build_source_graph(items)
    parent=list(range(len(items)))
    edge_reason={};edge_conf={}
    def find(x):
        while parent[x]!=x:
    ...
    for e in graph.edges:
        if e.confidence>=.7:
            union(e.source_index,e.target_index)
            edge_reason[(e.source_index,e.target_index)]=e.reason
            edge_conf[(e.source_index,e.target_index)]=e.confidence
    ...
    for indices in groups.values():
        seed="|".join(sorted(items[i].canonical_url or items[i].url for i in indices))
        chain="S"+hashlib.sha1(seed.encode()).hexdigest()[:10]
        group_conf=.0;reasons=[]
        for a in indices:
            for b in indices:
                if a<b:
                    c=edge_conf.get((a,b),edge_conf.get((b,a),0))
                    if c:
                        group_conf=max(group_conf,c)
                        reasons.append(edge_reason.get((a,b),edge_reason.get((b,a),"")))
        for i in indices:
            if len(indices)>1:
                out[i]=replace(out[i],source_chain_id=chain,source_chain_confidence=round(group_conf or .7,3),
                               source_chain_reason="; ".join(dict.fromkeys(x for x in reasons if x))[:240])
            else:
                out[i]=replace(out[i],source_chain_id=chain,source_chain_confidence=0.0,source_chain_reason="singleton")
    return out
...
class IndependenceAssessment:
    ...
    def conservative_count(self)->int:
        return self.certain_independent + self.likely_independent
...
def assess_independence(evidence:Sequence[Evidence])->IndependenceAssessment:
    ...
    groups={}
    for e in items:
        key=(e.independence_key,e.source_chain_id if e.source_chain_confidence>=.7 else "")
        groups.setdefault(key,[]).append(e)
    certain_same=sum(max(0,len(v)-1) for v in groups.values() if any(x.source_chain_confidence>=.9 for x in v))
    likely_same=sum(max(0,len(v)-1) for v in groups.values() if not any(x.source_chain_confidence>=.9 for x in v))
    count=len(groups)
    certain=sum(1 for v in groups.values() if len(v)==1 and v[0].source_chain_confidence==0)
    likely=max(0,count-certain)
...
def independent_source_count(evidence:Sequence[Evidence])->int:
    ...
    for i in range(len(items)):
        for j in range(i+1,len(items)):
            ...
            same_chain=bool(items[i].source_chain_id and items[i].source_chain_id==items[j].source_chain_id and
                            min(items[i].source_chain_confidence,items[j].source_chain_confidence)>=.7)
