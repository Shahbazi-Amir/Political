...
class StaticReviewerRegistry:
    def __init__(self, reviewers: Mapping[str, Any] | None = None) -> None:
        self._reviewers: dict[str, ReviewerIdentity] = {}
        for reviewer_id, value in (reviewers or {}).items():
            if isinstance(value, ReviewerIdentity):
                identity = value
            elif isinstance(value, Mapping):
                identity = ReviewerIdentity(str(reviewer_id), bool(value.get("active", True)), frozenset(str(x) for x in value.get("roles", ["reviewer"])))
            else:
                identity = ReviewerIdentity(str(reviewer_id))
            self._reviewers[identity.reviewer_id] = identity
    def add(self, reviewer_id: str, *, roles=None, active: bool = True) -> ReviewerIdentity:
        reviewer_id = str(reviewer_id).strip()
        if not reviewer_id: raise ValueError("reviewer_id is required")
        identity = ReviewerIdentity(reviewer_id, active, frozenset(roles or {"reviewer"}))
        self._reviewers[reviewer_id] = identity
        return identity
    def resolve(self, reviewer_id: str) -> ReviewerIdentity | None:
        return self._reviewers.get(str(reviewer_id).strip())
