...
from political_core.openai_reasoning import OpenAIReasoningProvider,ReasoningProviderError,ReasoningSchemaError,ReasoningTimeout
...
def test_non_retryable_auth_error_is_not_retried():
    client=Client([HTTPishError(401),Resp(good_json())]);p=OpenAIReasoningProvider("x",client=client,max_retries=1)
    with pytest.raises(ReasoningProviderError):p.evaluate("claim",evidence())
    assert client.responses.calls==1

def test_retryable_rate_limit_can_retry_once():
    client=Client([HTTPishError(429),Resp(good_json())]);p=OpenAIReasoningProvider("x",client=client,max_retries=1);d=p.evaluate("claim",evidence());assert d.verdict.value=="true";assert client.responses.calls==2
